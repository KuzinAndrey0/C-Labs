...
while test -f ../data/"$file"; do #Проверять все файлы с позитивными тестами
    ...
    fileFmt="pos_""$num""_fmt.txt"
    if [ -f ../data/"$fileKey" ]; then
        key="../data/""$fileKey"
    else
        key=""
    fi
    if [ -f ../data/"$fileFmt" ]; then
        fmt="../data/""$fileFmt"
    else
        fmt=""
    fi

    eval ./pos_case.sh ../data/"$file" ../data/"$fileOut" "$key" "$fmt";
    returnCode="$?"
...
while test -f ../data/"$file"; do #Проверять все файлы с негативными тестами    
    ...
    eval ./neg_case.sh ../data/"$file" "$key" "$fmt"
