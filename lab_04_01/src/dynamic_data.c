...
int add_global_data(void *ptr, int type)
{
    ...
    {
        void **test_ptr = realloc(global_data.ptr, global_data.current_mem + global_data.step_mem);
        if (test_ptr == NULL)
        {
            return 0;
        }

        global_data.ptr = test_ptr;
    }
...
int list_add_last(list_t *list, int value)
{
    list_node *new_node;
    list_node *ptr = list->head;
